...
+
. If the URL API requires your application to send query parameters in GET requests, specify the query parameters. For example, you may want to send user or database information in query parameters for the URL API to trigger the custom action workflow. Make sure these query parameters are defined as key-value pairs. 
