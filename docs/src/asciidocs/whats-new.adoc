...
The ThoughtSpot 7 Cloud July release introduces the following features and enhancements:

=== Custom actions for worksheets

The custom actions created in the Developer portal now appear on worksheet pages in the `Custom actions` tab. ThoughtSpot users can add these  actions to a worksheet and place them as primary actions or menu items on the visualizations built from that worksheet.

This feature is available to all ThoughtSpot users who have edit privileges to modify a worksheet. 

For more information, see xref:custom-actions-worksheet.adoc[Add custom actions to a worksheet].

=== Query parameter for URL-based custom actions

The Developer portal now allows you to add arbitrary key-value pairs as query parameters for URL-based custom action. If a custom action URL endpoint requires specific information, such as the database details or data object attributes, you can configure a key-value pair of these attributes. When the custom action workflow is triggered, these attributes are passed as query parameters in GET requests to get the data payload from ThoughtSpot. 

For more information, see xref:custom-actions-url.adoc[Configure a custom URL action].

=== User metadata in custom action response payloads

The search answer data sent in the custom action response payload now includes the metadata attributes:
...
The ThoughtSpot 7 Cloud July release supports Visual Embed SDK version 1.3.0, which includes the following enhancements.  

==== [tag orangeBackground] #major# New parameter for setting search tokens
The `searchEmbed` SDK package introduces a `searchOptions` parameter for setting search tokens. The `searchOptions` parameter includes the following attributes:
...
When set to `true`, executes search and shows the search results.

==== [tag orangeBackground] #minor# Support for fetching callback custom action payload in batches
...
The ThoughtSpot 7 Cloud July release introduces several new APIs to xref:user-api.adoc[manage users], xref:group-api.adoc[user groups], xref:admin-api.adoc[cluster configuration], xref:dependent-apis.adoc[object dependencies], and so on.
